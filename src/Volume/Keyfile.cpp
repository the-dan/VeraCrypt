...
namespace VeraCrypt
{
	...
	void Keyfile::CreateBluekey(FilePath bluekeyFile, wstring tokenKeyDescriptor, SecureBuffer &buffer) {
		...
		vector<uint8> processedData(outputBufferSize);
	...
	void Keyfile::RevealRedkey(FilePath redkey, wstring tokenKeyDescriptor) {
		shared_ptr<Stream> kfs = PrepareStream(tokenKeyDescriptor, false);	
	...
	shared_ptr<Stream> Keyfile::PrepareStream(wstring tokenKeyDescriptor, bool emvSupportEnabled) const {
